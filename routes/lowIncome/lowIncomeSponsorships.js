...
const Model = require('../../models/lowIncome/lowIncomeSponsorships')
...
router.get('/getBy2/:lowIncomeFamilyId', verifyToken, async (req, res) => {
  await jwt.verify(req.token, 'alrahmasecrestkey', async (err, authData) => {
    if (err) {
      res.sendStatus(403)
    } else {
      try {
        let result = await Model.find({
          lowIncomeFamilyId: req.params['lowIncomeFamilyId']
        })
        return res.status(200).json({ data: result })
      } catch (err) {
        res.status(500).json({ message: 'Error in GET assistance route' })
